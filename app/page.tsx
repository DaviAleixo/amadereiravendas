...
import { ArrowRight, ArrowUpRight, ChevronLeft, ChevronRight, Menu, X, Search } from 'lucide-react'
...
function Header() {
  ...
  return <header className="site-header"><a href="#top" className="brand" aria-label="Amadeireira, início"><img src="/logo1.webp" alt="Amadeireira" style={{ height: '40px', width: 'auto' }} /></a><nav className={`nav ${open ? 'is-open' : ''}`}><a href="#produtos" onClick={() => setOpen(false)}>Produtos</a><a href="https://www.instagram.com/amadeireira_?utm_source=ig_web_button_share_sheet&igsi=ZDNlZDc0MzIxNw==" target="_blank" rel="noreferrer" onClick={() => setOpen(false)}>Instagram <ArrowUpRight size={13} /></a><a className="nav-contact" href={generalWhatsappUrl()} target="_blank" rel="noreferrer" onClick={() => setOpen(false)}><img src="/wpp.png" alt="WhatsApp" style={{ width: '15px', height: '15px' }} /> WhatsApp</a></nav><button className="menu-button" aria-label="Abrir menu" onClick={() => setOpen(!open)}>{open ? <X size={22} /> : <Menu size={22} />}</button></header>
}

function Hero() { return <section className="hero" id="top"><div className="hero-copy"><p className="eyebrow">A beleza do natural</p><h1>Madeira que<br /><em>transforma</em> ambientes.</h1><p className="hero-description">Móveis marcantes, acabamento cuidadoso e a beleza natural da madeira em cada detalhe.</p><div className="hero-actions"><a className="button button-dark" href="#produtos">Conheça nossos produtos <ArrowRight size={16} /></a><a className="text-link" href={generalWhatsappUrl()} target="_blank" rel="noreferrer">Fale conosco <ArrowUpRight size={15} /></a></div></div><div className="hero-image"><Image src="/capa.webp" alt="Mesa de madeira em ambiente sofisticado" fill priority sizes="(max-width: 800px) 100vw, 57vw" /></div><span className="hero-mark">01 / AMADEIREIRA</span></section> }
...
function Footer() { return <footer><a href="#top" className="brand"><img src="/logo1.webp" alt="Amadeireira" style={{ height: '30px', width: 'auto' }} /></a><p>Peças que aproximam a natureza da casa.</p><div className="footer-links"><a href="https://www.instagram.com/amadeireira_?utm_source=ig_web_button_share_sheet&igsi=ZDNlZDc0MzIxNw==" target="_blank" rel="noreferrer">Instagram</a><a href={generalWhatsappUrl()} target="_blank" rel="noreferrer"><img src="/wpp.png" alt="WhatsApp" style={{ width: '15px', height: '15px' }} /> WhatsApp</a><a href="https://amadeireira.com.br/" target="_blank" rel="noreferrer">Site principal <ArrowUpRight size={13} /></a></div><small>© {new Date().getFullYear()} Amadeireira. Todos os direitos reservados.</small></footer> }
...
export default function Page() { 
  ...
  return (
    <main>
      ...
      <section className="catalog" id="produtos">
        ...
        {filtered.length > 0 ? (
          <div className="product-grid">
            {filtered.map((product, i) => <ProductCard key={product.id} index={i} product={product} onOpen={setModal} />)}
          </div>
        ) : (
      ...
      <Footer />
      <a className="floating-whatsapp" href={generalWhatsappUrl()} target="_blank" rel="noreferrer" aria-label="Falar com a Amadeireira pelo WhatsApp"><img src="/wpp.png" alt="WhatsApp" style={{ width: '22px', height: '22px' }} /></a>
